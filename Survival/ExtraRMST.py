# -*- coding: utf-8 -*-
"""
Created on Thu Apr 26 16:04:14 2018
...
class RandomForest(object):
    ...
    def makeTree(self, data, intervention, outcomes, bootstrapCut):
        ...
        tree = Root(data[:bootstrapCut,...], intervention[:bootstrapCut], 
                    outcomes[:bootstrapCut,...], self.numTry, self.minGroup, self.pval, 
...
class Root(object):
    def __init__(self,  data, intervention, outcomes, numTry, minGroup, pval, columnTypes, colNames):
        super().__init__()
...
class Node(object):
    ...
    def evaluateSplits(self):
        if len(self.rowIndex) >= self.root.minGroup*4 + 2:
        ...
        if len(uniqueVals) > 1:
            ...
            if len(L_intervention) > reg and len(R_intervention) > reg:
                ...
                if self.minSize(L_model, R_model) > self.root.minGroup:
                    m = len(self.rowIndex)
                    performance = abs(L_model.getRMSTdiff())*(L_model.totalC + L_model.totalT)/m
                    performance += abs(R_model.getRMSTdiff())*(R_model.totalC + R_model.totalT)/m
